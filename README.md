...
一个即开即用的 dotfiles 仓库。配置按软件模块管理，运行安装脚本后即可交互式选择要部署的模块，脚本会自动预览影响、备份已有配置、创建链接并验证结果。
...
- `Space`：选择或取消选择模块。
- `Enter`：确认选择并开始部署。
- 部署前会显示本次影响，包括目标 HOME、备份目录、将创建或替换的配置。
- 部署时会先备份已有目标，再创建链接。
- 部署后会验证结果并反馈。
...
新增流程会要求输入模块名称，例如 `nvim`、`git`、`alacritty`，然后进入交互式路径选择界面：
...
部署器会先初始化该模块的依赖，再按 manifest 行顺序创建链接。

## 🔁 兼容入口

`prepare.sh` 和 `deploy.sh` 仍然保留，用于兼容旧习惯：

- `prepare.sh`：兼容包装脚本，用于初始化模块依赖。
