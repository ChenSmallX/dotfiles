...
switch ($ScriptArgs[0]) {
    ...
    "help" { Show-Usage }
    "describe" {
        Show-DotfileImpact (Join-Path $ModuleDir "files\.vimrc") ".vimrc"
        Show-DotfileImpact (Join-Path $ModuleDir "files\.vim") ".vim"
    }
    "deploy" {
        Install-DotfileItem (Join-Path $ModuleDir "files\.vimrc") ".vimrc"
        Install-DotfileItem (Join-Path $ModuleDir "files\.vim") ".vim"
    }
    "verify" {
        $ok = (Test-DotfileItem (Join-Path $ModuleDir "files\.vimrc") ".vimrc")
        $ok = (Test-DotfileItem (Join-Path $ModuleDir "files\.vim") ".vim") -and $ok
