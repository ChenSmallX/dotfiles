...
switch ($ScriptArgs[0]) {
    ...
    "--help" { Show-Usage }
    "help" { Show-Usage }
    "describe" { Show-DotfileImpact (Join-Path $ModuleDir "files\.bashrc") ".bashrc" }
    "deploy" { Install-DotfileItem (Join-Path $ModuleDir "files\.bashrc") ".bashrc" }
    "verify" { if (-not (Test-DotfileItem (Join-Path $ModuleDir "files\.bashrc") ".bashrc")) { throw (Get-DotfilesMessage "验证失败" "verification failed") } }
