...
switch ($ScriptArgs[0]) {
    ...
    "--help" { Show-Usage }
    "help" { Show-Usage }
    "describe" { Show-DotfileImpact (Join-Path $ModuleDir "files\.profile") ".profile" }
    "deploy" { Install-DotfileItem (Join-Path $ModuleDir "files\.profile") ".profile" }
    "verify" { if (-not (Test-DotfileItem (Join-Path $ModuleDir "files\.profile") ".profile")) { throw (Get-DotfilesMessage "验证失败" "verification failed") } }
