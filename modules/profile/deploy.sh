...
MODULE_DIR="${DOTFILES_ROOT}/modules/profile"
...
case "${1:-}" in
  -h|--help|help)
    ...
    ;;
  describe)
    describe_link "${MODULE_DIR}/files/.profile" ".profile"
    ;;
  deploy)
    link_item "${MODULE_DIR}/files/.profile" ".profile"
    ;;
  verify)
    verify_link "${MODULE_DIR}/files/.profile" ".profile"
    ;;
  *)
