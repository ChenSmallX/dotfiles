...
switch ($ScriptArgs[0]) {
    ...
    "--help" { Show-Usage }
    "help" { Show-Usage }
    "describe" { Show-DotfileImpact (Join-Path $ModuleDir "files\.pip") ".pip" }
    "deploy" { Install-DotfileItem (Join-Path $ModuleDir "files\.pip") ".pip" }
    "verify" { if (-not (Test-DotfileItem (Join-Path $ModuleDir "files\.pip") ".pip")) { throw (Get-DotfilesMessage "验证失败" "verification failed") } }
